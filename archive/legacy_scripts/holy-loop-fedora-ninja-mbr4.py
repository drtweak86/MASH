...
def main():
    ...
    try:
        ...
        # ---------- 1) CLEAN & PARTITION (MBR, EXACTLY 4 PRIMARY) ----------
        banner("Wiping & Partitioning (MBR, 4 parts)", icon="🏗️")
        sh(f"wipefs -a {disk}")
        sh(f"parted -s {disk} mklabel msdos")

        # Use MiB units for deterministic endpoints.
        # Layout:
        #   start at 4MiB for alignment
        start_mib = 4
        p1_start = start_mib
        p1_end   = p1_start + EFI_SIZE_MIB

        p2_start = p1_end
        p2_end   = p2_start + BOOT_SIZE_MIB

        root_mib = tib_to_mib(ROOT_SIZE_TIB)
        data_mib = tib_to_mib(DATA_SIZE_TIB)

        p3_start = p2_end
        p3_end   = p3_start + root_mib

        p4_start = p3_end
        p4_end   = p4_start + data_mib

        sh(f"parted -s {disk} unit MiB mkpart primary fat32 {p1_start} {p1_end}")
        sh(f"parted -s {disk} set 1 boot on")  # MBR boot flag (works fine for FAT32 'EFI' partition)

        sh(f"parted -s {disk} unit MiB mkpart primary ext4  {p2_start} {p2_end}")
        sh(f"parted -s {disk} unit MiB mkpart primary btrfs {p3_start} {p3_end}")
        sh(f"parted -s {disk} unit MiB mkpart primary ext4  {p4_start} {p4_end}")
