...
fn download_with_progress(
    ...
    if let Some(total) = total_size {
        eprintln!("📥 {} ({}):", description, format_bytes(total));
    } else {
...
pub fn download_uefi_firmware(destination_dir: &Path) -> Result<PathBuf> {
    ...
    eprintln!("🔧 Downloading UEFI Firmware for Raspberry Pi 4...");
    ...
    eprintln!("📦 Extracting UEFI firmware...");
