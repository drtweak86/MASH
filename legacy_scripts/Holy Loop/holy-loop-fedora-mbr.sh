...
for c in parted wipefs mkfs.vfat mkfs.ext4 losetup rsync blkid awk sed grep pv btrfs; do need "$c"; done
...
echo ">>> Copying Fedora root (with progress bar)"
SIZE=$(du -sb "$SRCROOT" | awk '{print $1}')
rsync -aHAX --numeric-ids "$SRCROOT"/ /mnt/dst/root/ | pv -s "$SIZE" >/dev/null

echo ">>> Installing Pi4 UEFI firmware"
rsync -a --no-owner --no-group "$UEFI_DIR"/ /mnt/dst/boot/

echo ">>> Copying Fedora EFI loaders"
mkdir -p /mnt/dst/boot/EFI
rsync -a --no-owner --no-group /mnt/src/efi/EFI/ /mnt/dst/boot/EFI/
...
echo ">>> Empty cmdline (UEFI handles kernel)"
echo "" > /mnt/dst/boot/cmdline.txt
...
sync
echo "✅ DONE. Power off and boot Pi 4."
