...
pub fn download_uefi_firmware(destination_dir: &Path) -> Result<()> {
    ...
    let github_api_url =
        "https://api.github.com/repos/pftf-rpm-sbsa/RPi4-UEFI-Firmware/releases/latest";
    info!(
    ...
    let asset = response
        .assets
        .iter()
        .find(|a| a.name.starts_with("RPi4-UEFI-Firmware-") && a.name.ends_with(".zip"))
        .ok_or_else(|| {
            anyhow!("Could not find RPi4-UEFI-Firmware-vX.Y.zip asset in latest release")
        })?;
...
pub fn download_fedora_image(
    ...
    let download_url = format!(
        "{}/{}/{}/{}/images/{}",
        base_url, version, edition, arch, filename
    );
    ...
    let mut response_stream = client
        ...
        .context(format!(
            "Failed to download Fedora image from {}",
            download_url
        ))?;
